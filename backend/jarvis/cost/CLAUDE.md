...
requirement.** `cost/store.py`'s three tables ARE this rule, structurally:
...
- `model_prices` — used only to **calculate** a dollar figure from a measured count;
  `report.py`'s `calculate()` returns `null` (never a guess) for anything with no known
  price on record.

## `cost/store.py` (leaf) + `ops/trace.py`

`cost/store.py` is the raw table access, same discipline as every other `*-store.py` in
this project. `ops/trace.py` is the thin, always-safe seam every real call site actually
...
never break the turn/call it's measuring (same principle `self/self-capture.py`'s
...
event, alongside the existing `chunk`/`call`/`final`. `orchestrator/pipeline.py`'s per-step event
...
discarded.** `anthropic_adapter.py`'s `finalMessage()` already returns `.usage` (including
`cache_read_input_tokens`, which is what finally makes the adapter's own `cache_control`
breakpoint's real payoff measurable). `gemini_adapter.py`'s every streamed chunk carries
`usageMetadata` with CUMULATIVE totals — the last one seen is the real total, never
summed. `openai_compatible.py` needed one more change: `stream_options:
...
`tts/matching.py`'s `stream()` records character count once resolution succeeds (matches
how most providers actually bill: per character requested, not per chunk successfully
consumed by playback); `stt/deepgram.py`'s `connect()` records real connected duration
...
`adapters/openai_compatible.py`'s `inferBillingFromPricing()` already parses today and
...
`balances.py`) — **deliberately NOT routed through the Heartbeat's
`registerSource()`/`routeFinding()` pipeline**, since that mechanism exists to judge
whether something is worth interrupting the owner for (`decision.py` spends a real model
...
`report.py` is the read side — `usageBreakdown(sinceIso)` groups by `(provider,
...
plainly "X has no known price" instead of silently under-reporting. `check_spending.py`
...
exactly (`1000*0.000003 + 500*0.000015 = 0.0105`); `report.py`'s aggregation across two
separately-recorded events summed correctly on every field. `routing.py`'s integration
