// Tool: runs one of an installed Skill's own helper scripts through the
// sandbox (server/sandbox/) and returns its output. This is the one
// deliberate exception to skills/store/skill-files.js/read_skill_file.js's
// "Jarvis only ever reads a Skill's files, never executes them" rule — see
// server/skills/CLAUDE.md for why that rule exists (the scheduler/briefing
// run unattended with the confirm gate switched off, so "run it, ask first"
// would silently skip asking). This is the "separate, deliberately-designed
// feature" that rule pointed at.
//
...
import { getSkill, resolveSkillPath, readSkillFile, listSkillFiles } from '../skills/store/skill-files.js';
