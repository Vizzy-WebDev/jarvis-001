// Tool: creates a scheduled/repeating task from ordinary conversation
// ("remind me every weekday at 7am to..."). Always confirmed first (see
// capabilities.js's confirmation layer) — a wrong time or day is exactly
...
export default {
  name: 'schedule_task',
  // Meta tool: manages Jarvis's own scheduling. Attaching "schedule a
  // task" as the action of a scheduled task would be a confusing loop, so
  // this is left out of the task/briefing capability picker (capabilities.js's
  // listCapabilities()) even though it's still fully usable in conversation.
  meta: true,
  description:
